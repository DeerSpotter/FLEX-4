...
static BOOL FLEX4BetaResolveInProcessFLEXManagerIfPossible(void) {
    ...
    id candidate = ((id (*)(id, SEL))[flexManagerClass methodForSelector:@selector(sharedManager)])(flexManagerClass, @selector(sharedManager));
    if (!candidate) {
    ...
    if (!FLXWindowClass) {
        Class windowClass = NSClassFromString(@"FLEXWindow");
        if (windowClass) {
            static Class cachedFLEXWindowClass = Nil;
            cachedFLEXWindowClass = windowClass;
            FLXWindowClass = ^Class{ return cachedFLEXWindowClass; };
        }
    }
